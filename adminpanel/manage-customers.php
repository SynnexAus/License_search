<?php
	...
	if(isset($_REQUEST['btnSubmit']))
	{
		...
		{
			$ExQry.=" and c.contact_id = '".$_REQUEST['contact_id']."'";
		}
		
		if(isset($_REQUEST['customer_id']) && $_REQUEST['customer_id']!="")
		{
			$ExQry.=" and c.customer_id = '".$_REQUEST['customer_id']."'";
		}
		...
		if(isset($_REQUEST['resellercode']) && $_REQUEST['resellercode']!="")
		{
			$ExQry.=" and rc.reseller_code = '".$_REQUEST['resellercode']."'";
		}
		if(isset($_REQUEST['email']) && $_REQUEST['email']!="")
		{
			$ExQry.=" and c.email = '".$_REQUEST['email']."'";
		}
		
		if(isset($_REQUEST['start_date']) && $_REQUEST['start_date']!="")
		{	
			$ExQry.=" and c.expiration_date >= '".date("Y-m-d",strtotime($_REQUEST['start_date']))."'";
		}
		
		if(isset($_REQUEST['to_date']) && $_REQUEST['to_date']!="")
		{	
			$ExQry.=" and c.expiration_date <= '".date("Y-m-d",strtotime($_REQUEST['to_date']))."'";
		}
            ...
            <div class="box-body table-responsive table-res customers-tb">
              <form name="SearchForm" id="SearchForm" method="post" action="" autocomplete="off" enctype="multipart/form-data">
               ...
               <input class="form-control" id="license_number" name="license_number" value="<?=$_REQUEST['license_number'];?>" type="text" />
                ...
                <label>Reseller Code
                	<input type="text" class="form-control" id="resellercode" name="resellercode" value="<?=$_REQUEST['reseller_code'];?>" />
                </label>
                <div class="clear">&nbsp;</div>
                <label>Date Expired
              ...
              <form name="custFrm" id="custFrm" action="" method="post">
                ...
                <thead>
                  <tr>
                    ...
                    <th>Licence ID</th>
                    <th class="licNum">Licence number</th>
                ...
                <tbody>
                  <?php 
					...
					//echo "SELECT c.*,rc.reseller_company, rc.reseller_code FROM customers c LEFT join reseller_code_mapping rc on c.contact_id=rc.avg_id where 1 = 1 $ExQry"; 
					list($result,$pglink,$pgcombo)=$mfp->mf_admin_paging("SELECT c.*,rc.reseller_company, rc.reseller_code FROM customers c LEFT join reseller_code_mapping rc on c.contact_id=rc.avg_id where 1 = 1 $ExQry ",50);
