<a href="dashboard.php" class="logo">AVG Adminpanel</a> 
<nav class="navbar navbar-static-top" role="navigation">
                <!-- Sidebar toggle button-->
                <a href="#" class="navbar-btn sidebar-toggle" data-toggle="offcanvas" role="button">
                    <span class="sr-only">Toggle navigation</span>
                    <span class="icon-bar"></span>
                    <span class="icon-bar"></span>
                    <span class="icon-bar"></span>
                </a>
                <div class="navbar-right">
                    <ul class="nav navbar-nav">
                        <!-- Messages: style can be found in dropdown.less-->
                        
                        <!-- User Account: style can be found in dropdown.less -->
                        <li class="dropdown user user-menu">
                            <a href="#" class="dropdown-toggle" data-toggle="dropdown">
                                <i class="glyphicon glyphicon-user"></i>
                                <span><?=ucfirst($mfp->mf_getvalue("admin","username","id",$_SESSION['Admin_UserID']));?> <i class="caret"></i></span>
                            </a>
                            <ul class="dropdown-menu">
                                <!-- User image -->
                                <li class="user-header bg-light-blue">
                                    <?php /*?><?php 
									$img=$mfp->mf_getValue("admin","image_path","id",$_SESSION['Admin_UserID']);
										if(is_file("../uploads/profile/".$img))
										{ ?>
											<img src="../uploads/profile/<?=$img;?>" class="img-circle" alt="User Image"/>                           
										<?php }else{ ?>
											<img src="../uploads/profile/no_avatar.jpg" class="img-circle" alt="User Image"/>
										<?php	} ?><?php */?>
                                    <p>
                                       <?=ucfirst($mfp->mf_getvalue("admin","username","id",$_SESSION['Admin_UserID']));?>
                                       
                                    </p>
                                </li>                             
                                
                                <li class="user-footer">
                                    <div class="pull-left">
                                        <a href="profile.php" class="btn btn-default btn-flat">Profile</a>
                                    </div>
                                    <div class="pull-right">
                                        <a href="logout.php" class="btn btn-default btn-flat">Sign out</a>
                                    </div>
                                </li>
                            </ul>
                        </li>
                    </ul>
                </div>
            </nav>